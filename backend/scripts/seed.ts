...
async function main(): Promise<void> {
  ...
  console.log('[OK] Catalogo de categorias, laboratorios y unidades');

  // Productos de ejemplo con stock
  const productsData = [
    { sku: 'PARA-500', name: 'Paracetamol 500mg', categoryId: analgesicos.id, labId: labA.id, unitId: caja.id, presentation: 'caja', price: 8.5, cost: 5.0, min: 20 },
    { sku: 'IBU-400', name: 'Ibuprofeno 400mg', categoryId: analgesicos.id, labId: labA.id, unitId: caja.id, presentation: 'caja', price: 12.0, cost: 7.5, min: 15 },
    { sku: 'AMO-500', name: 'Amoxicilina 500mg', categoryId: antibioticos.id, labId: labB.id, unitId: caja.id, presentation: 'caja', price: 25.0, cost: 16.0, min: 10 },
    { sku: 'VIT-C', name: 'Vitamina C 1g efervescente', categoryId: (await prisma.category.findUnique({ where: { name: 'Vitaminas' } }))!.id, labId: labB.id, unitId: frasco.id, presentation: 'frasco', price: 35.0, cost: 22.0, min: 8 },
    { sku: 'JAR-ABR', name: 'Jarabe para la tos 120ml', categoryId: (await prisma.category.findUnique({ where: { name: 'Analgesicos' } }))!.id, labId: labA.id, unitId: frasco.id, presentation: 'jarabe', price: 30.0, cost: 19.0, min: 6 },
  ];
